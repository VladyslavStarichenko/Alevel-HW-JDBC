...
public class ProductDao {
    ...
    public static void updateProduct(Connection connection, String name, Integer price ,Integer category_Id, Integer product_Id) {
        String sqlUpdate = "UPDATE product  SET  product_name = ?, price = ?, category_id = ? WHERE product_id = ?";
        PreparedStatement statement;
        {
            try {
                ...
                int rows = statement.executeUpdate();
                if (rows > 0)
                    System.out.println("Product name with id: "+ product_Id + " has been updated!");
                else
    ...
    public static void createIfCategoryExists(Connection connection, String name, Integer price, Integer categoryId) {
        ...
        String sqlInsertion = "INSERT INTO product (product_name, price, category_id) VALUES (?, ?, ?)";
        creatingMethod(connection, name, price, categoryId, sqlInsertion);
    }

    private static void creatingMethod(Connection connection, String name, Integer price, Integer categoryId, String sqlInsertion) {
        PreparedStatement statement;
        {
            try {
    ...
    public static List<Product> getAllProducts(Connection connection) {
        ...
        try {
            ...
            while (resultSet.next()) {
                Product product = new Product(resultSet.getInt(1),resultSet.getString(2),resultSet.getInt(3),resultSet.getInt(4));
